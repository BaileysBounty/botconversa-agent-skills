...
Quem preferir um comando de terminal pode instalar a release imutável `v0.1.0`:

```bash
gh repo clone BaileysBounty/botconversa-agent-skills "$HOME/.botconversa-agent-skills-v0.1.0" -- --branch v0.1.0 &&
"$HOME/.botconversa-agent-skills-v0.1.0/install.sh"
```
...
Uma nova versão deve ser instalada a partir de uma nova tag, depois de revisar as notas da release. A v0.1.0 não faz atualização automática do conteúdo ativo.

Para trocar de release com segurança, remova primeiro apenas os links da versão atual e então instale a nova tag:

```bash
"$HOME/.botconversa-agent-skills-v0.1.0/uninstall.sh"
```
